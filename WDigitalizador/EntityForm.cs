...
namespace WConsultas
{
    public partial class EntityForm : Form
    {
        ...
        public EntityForm(DocumentoClsNivel unDocumentoClsNivel)
        {
            InitializeComponent();
            ...
            findTipoDoc(miDocumento.Id_tipo);
        }

        private void findTipoDoc(int tipoDoc)
        {

            for (int i = 0; i < comboBoxTipoDoc.Items.Count; i++)
            {

                TipoDocumentoNivel uo = (TipoDocumentoNivel)comboBoxTipoDoc.Items[i];

                if (uo.Id_tipodoc == tipoDoc)
                {
